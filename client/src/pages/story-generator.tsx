...
import { removeWhiteBackground } from "@/lib/canvas-utils";
...
export default function StoryGenerator() {
  ...
  const mortgageType = form.watch("mortgageType");
  ...
  useEffect(() => {
    if (loadedTemplate && !form.formState.isDirty) {
      form.reset({
        ...
        firstYearPayment: loadedTemplate.firstYearPayment || 0,
        secondYearRate: loadedTemplate.secondYearRate || 0,
        secondYearPayment: loadedTemplate.secondYearPayment || 0,
      });
  ...
  // Handler for floor plan upload with automatic white background removal
  const handleFloorPlanSelect = async (file: File) => {
    try {
      // Show loading toast
      toast({
        title: "Обработка изображения...",
        description: "Удаляем белый фон и обрезаем края",
      });
      
      // Process the image to remove white background and crop edges
      const processedFile = await removeWhiteBackground(file);
      setFloorPlan(processedFile);
      
      // Show success toast
      toast({
        title: "Планировка загружена",
        description: "Белый фон удален, края обрезаны",
      });
    } catch (error) {
      ...
      toast({
        title: "Планировка загружена",
        description: "Загружена без обработки",
        variant: "default",
      });
    }
  };
                ...
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <div className="pt-4">
                    ...
                    <div className="space-y-4">
                      ...
                      {/* Additional fields for tranche mortgage */}
                      {mortgageType === "tranche" && (
                        <>
                          <FormField
                            control={form.control}
                            name="firstYearPayment"
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-lg font-bold text-primary">Платёж первого года (₽)</FormLabel>
                                <FormControl>
                                  <Input
                                    type="number"
                                    step="0.01"
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-lg font-bold text-primary">Ставка с 2-го года (%)</FormLabel>
                                <FormControl>
                                  <div className="relative">
                                    <Input
                                      type="number"
                          ...
                          <FormField
                            control={form.control}
                            name="secondYearPayment"
                            render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-lg font-bold text-primary">Платёж с 2-го года (₽)</FormLabel>
                                <FormControl>
                                  <Input
                                    type="number"
                                    step="0.01"
                  ...
                  <div className="pt-4">
                    ...
                    <FileUpload
                      onFileSelect={handleFloorPlanSelect}
                      accept="image/*,.pdf"
                      maxSize={10 * 1024 * 1024}
