...
export async function drawStoryCanvas(
  ...
  if (floorPlan) {
    try {
      ...
      const planWidth = floorPlanPosition?.width || 600;
      const planHeight = floorPlanPosition?.height || 450;
      const planX = floorPlanPosition?.x || (CANVAS_WIDTH / 2 - planWidth / 2);
      const planY = floorPlanPosition?.y || 450;
      
      // Draw floor plan image - use full container size for better visibility
      const { x, y, width, height } = calculateImageFit(planImg, planWidth, planHeight);
      ctx.shadowColor = "rgba(0, 0, 0, 0.8)";
      ctx.shadowBlur = 10;
      ctx.drawImage(planImg, planX + x, planY + y, width, height);
      ctx.shadowBlur = 0;
  ...
  // Right side - Monthly payment (different for tranche mortgage)
  ctx.textAlign = "right";
  
  if (propertyData.mortgageType === "tranche") {
    // Tranche mortgage - show both year payments
    ctx.font = "28px Inter, sans-serif";
    ctx.fillStyle = "#9CA3AF";
    ctx.fillText("Платёж 1 года:", CANVAS_WIDTH - cardPadding - 30, cardY + 50);
    
    ctx.font = "bold 40px Inter, sans-serif";
    ctx.fillStyle = "white";
    ctx.fillText(
      `${formatCurrency(propertyData.firstYearPayment || 0)} ₽`,
    ...
    ctx.font = "28px Inter, sans-serif";
    ctx.fillStyle = "#9CA3AF";
    ctx.fillText("Платёж с 2 года:", CANVAS_WIDTH - cardPadding - 30, cardY + 125);
    
    ctx.font = "bold 40px Inter, sans-serif";
    ctx.fillStyle = "white";
    ctx.fillText(
      `${formatCurrency(propertyData.secondYearPayment || 0)} ₽`,
      CANVAS_WIDTH - cardPadding - 30,
      cardY + 160
    );
  } else {
    // Regular mortgage - show monthly payment
    ctx.font = "32px Inter, sans-serif";
    ctx.fillStyle = "#9CA3AF";
  ...
  // Bank rate - Enhanced visibility (different for tranche mortgage)
  ctx.textAlign = "left";
  ctx.font = "24px Inter, sans-serif";
  ctx.fillStyle = "#9CA3AF";
  
  if (propertyData.mortgageType === "tranche") {
    // Show both rates for tranche mortgage
    ctx.fillText("Ставка 1 года:", cardPadding + 30, cardY + 190);
    
    const firstRateText = `${propertyData.bankRate || 0}%`;
    ctx.font = "bold 30px Inter, sans-serif";
    const firstRateMetrics = ctx.measureText(firstRateText);
    const firstRateBoxWidth = firstRateMetrics.width + 15;
    const firstRateBoxHeight = 40;
    const firstRateBoxX = cardPadding + 160;
    const firstRateBoxY = cardY + 165;
    
    // First rate highlight background
    ctx.fillStyle = "#3B82F6";
    ctx.fillRect(firstRateBoxX, firstRateBoxY, firstRateBoxWidth, firstRateBoxHeight);
    
    // First rate text
    ctx.fillStyle = "white";
    ctx.fillText(firstRateText, firstRateBoxX + 8, firstRateBoxY + 28);
    
    // Second rate
    ctx.font = "24px Inter, sans-serif";
    ctx.fillStyle = "#9CA3AF";
    ctx.fillText("Ставка с 2 года:", cardPadding + 30, cardY + 240);
    
    const secondRateText = `${propertyData.secondYearRate || 0}%`;
    ctx.font = "bold 30px Inter, sans-serif";
    const secondRateMetrics = ctx.measureText(secondRateText);
    const secondRateBoxWidth = secondRateMetrics.width + 15;
    const secondRateBoxHeight = 40;
    const secondRateBoxX = cardPadding + 190;
    const secondRateBoxY = cardY + 215;
    
    // Second rate highlight background
    ctx.fillStyle = "#EF4444";
    ctx.fillRect(secondRateBoxX, secondRateBoxY, secondRateBoxWidth, secondRateBoxHeight);
    
    // Second rate text
    ctx.fillStyle = "white";
    ctx.fillText(secondRateText, secondRateBoxX + 8, secondRateBoxY + 28);
...
// Function to crop transparent edges from image
function cropTransparentEdges(canvas: HTMLCanvasElement, ctx: CanvasRenderingContext2D): HTMLCanvasElement {
  ...
  // Copy the non-transparent region to new canvas
...
export function removeWhiteBackground(file: File): Promise<File> {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        // Check if pixel is close to white (with some tolerance for near-white pixels)
      ...
      // Crop transparent edges to minimize image size
      const croppedCanvas = cropTransparentEdges(canvas, ctx);
      
      // Convert cropped canvas to blob and then to File
